...
/**
 * Retorna todos os horários já ocupados (status "agendado") de hoje em
 * diante, usado tanto pra avisar a Wanessa quanto pra conferir conflito
 * antes de salvar um agendamento novo.
 */
async function getHorariosOcupados() {
  ...
  return rows
    .map((row, i) => linhaParaObjeto(row, i + 2))
    .filter((p) => p.status === 'agendado' && p.agendamentoData && p.agendamentoData >= hoje)
    .map((p) => ({
      telefone: p.telefone,
